...
table = [ [ prefix minus      $ Unary  UnaryPrefix  Decrement
          , prefix plus       $ Unary  UnaryPrefix  Increment
          , postfix minusminus $ Unary  UnaryPostfix Decrement
          , postfix plusplus   $ Unary  UnaryPostfix Increment ]
        , [ prefix  minus      $ Unary  UnaryPrefix  Negate
...
prefix' :: Parser b -> (a -> NodeAnnotation -> a) -> Operator Parser a
prefix'  name f = Prefix $ manyUnaryOp $ (getNA <&> flip f) <* name
